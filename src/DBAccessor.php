    ...
    class DBAccessor {
        ...
        function getQuizQuestions($username, $quizLength) {
            ...
            $stmt = $db->prepare("SELECT qKanji, qWord, qReading
                                    ...
                                    (SELECT k.kanji AS qKanji, w.word AS qWord,
                                            w.reading AS qReading,
                                            (wei.literacy_weight * sk.retention_score -
                                                (wei.total_q_weight/sk.total_questions_asked)) AS priority
                                        FROM student s
        ...
        */
        function getQuizAnswers($kanji, $word, $answerCount) {
            ...
            $stmt = $db->prepare("SELECT DISTINCT a.word, a.reading
                                    FROM (
                                    ...
                                    LIMIT ?");
            $stmt->bind_param("sssssi", $word, $kanji, $word, $kanji, $word, $answerCount);
            $stmt->execute();
            $results = $stmt->get_result();
